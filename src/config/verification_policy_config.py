...
# CONFIG-LIMPIEZA: este módulo definía y validaba un bloque de ~16 campos
# de política de "agentic retrieval en verificación"
# (independent_retrieval_enabled, retrieval_strategy,
# retrieval_rounds_by_intensity, light_retrieval_reason_codes,
# max_queries_per_claim, top_k_per_retriever,
# max_total_candidates_per_claim, max_final_evidence_per_claim,
# max_candidates_per_source, max_evidence_chars_per_claim, rrf_k,
# allow_corpus_wide_retrieval, allow_corpus_wide_contradiction,
# allow_corpus_wide_transversal, stop_when_structural_coverage_satisfied,
# stop_when_no_new_pairs, authorized_source_legacy_fallback_enabled) que
# tenían CERO consumidores reales en todo src/ (confirmado por grep
# exhaustivo; ver también el docstring de agentic_retrieval_policy_config.py,
# que ya documentaba lo mismo para 3 de estos campos). Retirados por
# completo: no eran caps activos sobre ningún flujo real, solo
# configuración vestigial. El límite real de candidatos en Stage 07 es
# ``fetch_k``/``effective_top_k_max`` en agentic_retrieval_policy_config.py.
