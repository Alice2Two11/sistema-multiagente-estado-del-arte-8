...
from src.tools.shared.bool_parsing import to_bool as _to_bool
...
def decide_extraction_rebuild(
    *,
    ...
    Delega en ``resolve_stage_reuse_decision``
    (``src/tools/shared/stage_reuse.py``) -- consolidación de la decisión
    "reutilizar vs reconstruir" compartida ahora con 05/06. Firma y claves
    de retorno de esta función sin cambios (sigue siendo lo que consume
    ``extraction_agent.py``); el comportamiento real tampoco cambia --
    mismo orden de prioridad, mismo mensaje de ``RuntimeError``, misma
    fórmula de ``should_rebuild_extraction``."""

    decision = resolve_stage_reuse_decision(
        force_rebuild=force_rebuild_extraction,
        previous_manifest=previous_manifest,
        outputs_exist=stage_outputs_exist,
        current_fingerprint=current_fingerprint,
        auto_rebuild=auto_rebuild_extraction,
        auto_rebuild_disabled_message=(
            "La extracción necesita regenerarse, "
            "pero EXTRACTION_POLICY['auto_rebuild'] es False."
        ),
    )
    return {
        "extraction_status": decision["status"],
        "rebuild_required": decision["rebuild_required"],
        "should_rebuild_extraction": decision["should_rebuild"],
    }
